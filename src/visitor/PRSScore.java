...
import xmlsimilarity.VectorElementVerification;
import xmlsimilarity.VectorElement;
...
public class PRSScore
  ...
  private int mode = WordBasedCosineSimilarity.VERIFICATION;
  private int unit = WordBasedCosineSimilarity.WORD;
  public PRSScore() {

  }

  public PRSScore(int a, int ab, int ac, float sim) {
  ...
  public String toString() {
    String output = precision + ":(p):" + recall + ":(r):" + sim + ":(s)";
    return output;
  }

  public Score addition(Score score) {
    PRSScore sum = new PRSScore();
    sum.setPrecision(precision + ( (PRSScore) score).getPrecision());
    sum.setRecall(recall + ( (PRSScore) score).getRecall());
    sum.setSim(sim + ( (PRSScore) score).getSim());
    return sum;
  }
  ...
  public Score score(ElementComponent ec1) {
    ...
    PRSScore score = new PRSScore(a, ab, ac, simi);
    return score;
  }
  /**
   * turn list to a vector of VectorElements for similary calculation
   * @param list ArrayList
   * @param tag String
   * @return Vector
   */
  private Vector constructVector(ArrayList list, String tag){
    Vector v = new Vector();
    int size = list.size();
    for(int i = 0; i < size; i++){ //instance number
       Vector insts = (Vector)list.get(i);
       Enumeration en = insts.elements();
       while(en.hasMoreElements()){
         String seg = (String)en.nextElement();
         addToVector(seg, tag, WordBasedCosineSimilarity.WORD, i, v);
       }
     }
    return v;
  }
   ...
   * @param unit int
   * @param position int : which instance the token appeared in
   * @return Vector
   */
  private void addToVector(String text, String parenttag, int unit, int position, Vector v){
    ...
    while(m.lookingAt()){
      ...
      text = m.group(3);
      String[] tokens = xmlsimilarity.Utilities.normalizedTokens(seg, unit);
      for(int i = 0; i < tokens.length; i++){
        VectorElement ve = this.mode == WordBasedCosineSimilarity.VERIFICATION ? new VectorElementVerification(tokens[i], position, parenttag+"/"+tag) : new VectorElement(tokens[i], position, tag) ;
        v.add(ve);
      }
      m = p.matcher(text);
    }
    if(text.compareTo("") != 0){
      String[] tokens = xmlsimilarity.Utilities.normalizedTokens(text, unit);
      for (int i = 0; i < tokens.length; i++) {
        VectorElement ve = this.mode == WordBasedCosineSimilarity.VERIFICATION ?
            new
            VectorElementVerification(tokens[i], position, parenttag + "/" + parenttag) :
            new VectorElement(tokens[i], position, parenttag);
        v.add(ve);
      }
    }
 }
  ...
  /**
   * compute similarity score for ec
   * @param ec ElementComponent
   * @return float
   */
  private float simScore(ElementComponent ec) {
      String tag = ec.getTag();
      if(tag.compareTo("fruits") == 0){
        System.out.print("");
      }
      //1. create a vector of "VectorElement" from answers
      ArrayList list = ec.getAnswers();
      Vector v1 = constructVector(list, tag);
      //2. do the same for markeds
      list = ec.getMarkeds();
      Vector v2 = constructVector(list, tag);
      //3. find the wbcs between the two files
      WordBasedCosineSimilarity wbcs = new WordBasedCosineSimilarity(mode);
      return wbcs.compute(v1, v2);
    }
    ...
    while(it.hasNext()){
      ...
      Enumeration en = insts.elements();
      while(en.hasMoreElements()){
        String seg = ((String)en.nextElement()).trim();
        if(seg.replaceAll("^\\s+","").indexOf("<") != 0 && seg.compareTo("") != 0){
