...
public class WordBasedCosineSimilarity {
  ...
  public static int CLAUS = 2; //unit
  ...
  public float compute(Vector v1, Vector v2) {
  ...
  protected float cosinSimilarity(Vector v1, Vector v2){
    ...
    for(int i = 0; i < len1; i++){
      VectorElement ve1 = (VectorElement) v1.get(i);
      //System.out.println("answer: "+ve1.toString());
      for (int j = 0; j < len2; j++) {
        if (this.mode == VERIFICATION) {
          VectorElementVerification ve2 = (VectorElementVerification) v2.get(j);
          if(ve1.getLocation() == ve2.getLocation()){
            float s = ( (VectorElementVerification) v1.get(i)).similarity( (
                VectorElementVerification)v2.get(j));
            dividend += s;
            if (s > 0) { //can only match once, jump out once it matches
              //System.out.println("marked match: "+ve2.toString());
              break;
            }
          }
        }
        else {
          VectorElement ve2 = (VectorElement) v2.get(j);
          if(ve1.getLocation() == ve2.getLocation()){
            float s = ve1.similarity(ve2);
            dividend += s;
            if (s > 0) { //can only match once, jump out once it matches
              break;
            }
    ...
    for (Node node = root.getFirstChild(); node != null;
      ...
      if (node.getNodeType() == node.ELEMENT_NODE) {
        if (isLeafElement(node)) {
          ...
          position += wordcount(Utilities.normalize(node.toString()));
        }
        else {
          traverse(node, v);
        }
  ...
  private void addToVector(Node node, Vector vector, int position){
    String text = Utilities.normalize(node.toString());
    String[] tokens = Utilities.normalizedTokens(text, unit);
